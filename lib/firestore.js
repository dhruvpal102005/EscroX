 ...
 * Supports both service (URL link + optional screenshot) and product (tracking ID) types,
 * as well as optional AI verification data.
 */
export async function submitMilestone(contractId, milestoneId, value, evidenceImageUrl, freelancerName, aiVerification = null, courier = null) {
    // Detect if caller is passing workType string (legacy) or AI object (new)
    const isLegacyCall = typeof aiVerification === 'string';
    const workType = isLegacyCall ? aiVerification : null;
    const aiResult = isLegacyCall ? null : aiVerification;
    const isProduct = workType === 'product';

    const updatePayload = {
        status: 'Submitted',
        submittedAt: serverTimestamp(),
        // Service: store as evidenceUrl link; Product: store as trackingId
        evidenceUrl: isProduct ? null : value,
        evidenceImageUrl: isProduct ? null : (evidenceImageUrl || null),
        trackingId: isProduct ? value : null,
        courier: isProduct ? courier : null,
    };

    // Attach AI verification fields if provided
    if (aiResult) {
        updatePayload.aiSummary     = aiResult.summary     || '';
        updatePayload.aiConfidence  = aiResult.confidence  || 'low';
        updatePayload.aiIsRelevant  = aiResult.isRelevant !== undefined ? aiResult.isRelevant : true;
        updatePayload.aiFlags       = aiResult.flags       || [];
    }

    await updateDoc(doc(db, 'contracts', contractId, 'milestones', milestoneId), updatePayload);
    await updateContractStatus(contractId, 'Inspection');

    const auditAction = isProduct
        ? `Shipment submitted — Tracking ID: ${value} via ${courier}`
        : `Milestone submitted — Delivery link: ${value || 'provided'}`;

    await addAuditLog(contractId, auditAction, `${freelancerName} (Freelancer)`, 'upload');

    // Add AI verification audit entry if available
    if (aiResult?.summary) {
        const confidenceEmoji = aiResult.confidence === 'high' ? '🟢' : aiResult.confidence === 'medium' ? '🟡' : '🔴';
        await addAuditLog(
            contractId,
            `🤖 AI Verification ${confidenceEmoji} — ${aiResult.summary.split('.')[0]}.`,
            'EscroX AI Verifier',
            'shield'
        );
    }
...
export async function rejectMilestone(contractId, milestoneId, title, clientName, reason) {
    const contractRef = doc(db, 'contracts', contractId);
    const contractSnap = await getDoc(contractRef);
    const contractData = contractSnap.data();

    const newRejectionCount = (contractData.rejectionCount || 0) + 1;
    const totalValue = contractData.totalValue || 0;
    let penalty = 0;

    if (newRejectionCount > 3) {
        penalty = totalValue * 0.1;
    }

    // Update milestone
    await updateDoc(doc(db, 'contracts', contractId, 'milestones', milestoneId), {
        status: 'Rejected', // Changed from Pending to Rejected to differentiate
        evidenceUrl: null,
        trackingId: null,
        courier: null,
        submittedAt: null,
    ...
    await updateDoc(contractRef, {
        ...
        totalPenalty: (contractData.totalPenalty || 0) + penalty,
        updatedAt: serverTimestamp()
    });

    const penaltyText = penalty > 0 ? ` (Incurred $${penalty.toFixed(2)} penalty)` : '';
    await addAuditLog(
        contractId,
        `"${title}" Rejected — ${reason.slice(0, 50)}...${penaltyText}`,
        `${clientName} (Client)`,
        'XCircle'
    );
}
