 ...
 * @param {string} value       - For service: URL link. For product: tracking ID.
 * @param {string} freelancerName
 * @param {'service'|'product'} workType
 * @param {string} [courier]   - TrackingMore courier slug (product only)
 */
export async function submitMilestone(contractId, milestoneId, value, freelancerName, workType = 'service', courier = null) {
    const isProduct = workType === 'product';

    const updatePayload = {
        status: 'Submitted',
        submittedAt: serverTimestamp(),
        // Keep evidenceUrl for backward compat (shown as link for service)
