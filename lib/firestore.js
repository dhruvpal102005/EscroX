...
export async function submitMilestone(contractId, milestoneId, evidenceUrl, evidenceImageUrl, freelancerName, aiVerification) {
    ...
    // Store AI verification data if available
    if (aiVerification) {
        milestoneUpdate.aiSummary = aiVerification.summary || '';
        milestoneUpdate.aiConfidence = aiVerification.confidence || 'low';
        milestoneUpdate.aiIsRelevant = aiVerification.isRelevant !== undefined ? aiVerification.isRelevant : true;
        milestoneUpdate.aiFlags = aiVerification.flags || [];
    }

    await updateDoc(doc(db, 'contracts', contractId, 'milestones', milestoneId), milestoneUpdate);
    ...
    // Add AI verification audit entry
    if (aiVerification?.summary) {
        const confidenceEmoji = aiVerification.confidence === 'high' ? '🟢' : aiVerification.confidence === 'medium' ? '🟡' : '🔴';
        await addAuditLog(
            contractId,
            `🤖 AI Verification ${confidenceEmoji} — ${aiVerification.summary.split('.')[0]}.`,
            'EscroX AI Verifier',
            'shield'
        );
    }
