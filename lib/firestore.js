...
export async function createContract({ clientUid, clientName, clientEmail, clientCountry,
    freelancerName, freelancerEmail, freelancerCountry, title, totalValue, currency,
    deadline, milestones }) {
    const data = {
    ...
    data.txHash = generateTxHash(data);
    ...
    for (const ms of milestones) {
        await addDoc(collection(db, 'contracts', ref.id, 'milestones'), {
            ...ms, status: 'Pending', evidenceUrl: null, createdAt: serverTimestamp()
        });
    }
...
export async function addAuditLog(contractId, action, actor, icon = 'shield') {
    await addDoc(collection(db, 'contracts', contractId, 'auditLog'), {
