...
const NewContract = () => {
    ...
    const addMilestone = () => setForm(f => ({ ...f, milestones: [...f.milestones, { title: '', amount: '' }] }));
    const removeMilestone = (i) => setForm(f => ({ ...f, milestones: f.milestones.filter((_, idx) => idx !== i) }));
    const updateMilestone = (i, key, value) => setForm(f => {
    ...
    return (
        <div className="min-h-screen bg-surface">
            <div className="max-w-2xl mx-auto px-6 py-10">
                ...
                <p className="text-slate-400 text-sm mb-8">Define parties, milestones, and timeline. Funds are locked until each milestone is approved.</p>
                    ...
                    <Card className="p-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input required className="input" placeholder="e.g. Mobile App Development" value={form.title}
                                ...
                                <div key={i} className="flex items-center gap-3">
                                    ...
                                    <div className="flex items-center gap-1 border border-slate-200 rounded-xl px-3 bg-slate-50">
                                        ...
                                        <input required type="number" min="1" className="w-24 py-2.5 bg-transparent text-sm text-slate-900 outline-none"
                                            placeholder="0" value={m.amount} onChange={e => updateMilestone(i, 'amount', e.target.value)} />
                                    </div>
                                    {form.milestones.length > 1 && (
                                        <button type="button" onClick={() => removeMilestone(i)} className="text-slate-300 hover:text-red-400 transition-colors">
                                            <Trash2 size={15} />
                                        </button>
                                    )}
                                </div>
